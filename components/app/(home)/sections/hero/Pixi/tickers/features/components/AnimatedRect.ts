...
export default function AnimatedRect(props: Props) {
  ...
  const render = () => {
    ...
    // Begin fill before drawing shapes for PIXI v6 compatibility
    g.beginFill(p.color as number, 1);

    if (p.type !== "arc") {
      // PIXI Graphics uses `drawRoundedRect` for rounded rectangles.
      // Replace legacy `roundRect` call with the correct API.
      g.drawRoundedRect(
      ...
      // Use drawCircle for full arc (circle) to ensure compatibility.
      g.drawCircle(p.x, p.y, p.width / 2);
    }
