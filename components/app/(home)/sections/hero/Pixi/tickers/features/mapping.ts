...
export default async function mapping(props: Props) {
  ...
  lines.forEach((line) => {
    try {
      if (!line.graphic.destroyed) line.graphic.destroy();
    } catch (e) {
      console.warn("Failed to destroy line graphic", e);
    }
  });
  rects.forEach((rect) => {
    try {
      if (!rect.graphic.destroyed) rect.graphic.destroy();
    } catch (e) {
      console.warn("Failed to destroy rect graphic", e);
    }
  });
  dots.forEach((dot) => {
    try {
      if (!dot.graphic.destroyed) dot.graphic.destroy();
    } catch (e) {
      console.warn("Failed to destroy dot graphic", e);
    }
  });
}
