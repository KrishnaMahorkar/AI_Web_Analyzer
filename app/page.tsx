...
export default function StyleGuidePage() {
  ...
  // Check for API keys and Rate Limit on mount
  useEffect(() => {
    // Check config
  ...
  const handleAnalysis = async () => {
    ...
    if (isRateLimited) {
      ...
      alert(`Daily limit reached. Please try again in ${hours} hours.`);
      return;
    }
              ...
              <div className="max-w-552 mx-auto w-full relative z-[11] lg:z-[2] rounded-20 -mt-30 lg:-mt-30">
                ...
                <div className="p-16 flex gap-12 items-center w-full relative">
                  ...
                  <input
                    ...
                    className={`flex-1 bg-transparent text-body-input text-accent-black placeholder:text-black-alpha-48 focus:outline-none focus:ring-0 focus:border-transparent ${urlError ? 'text-heat-200' : ''} ${isRateLimited ? 'opacity-50 cursor-not-allowed' : ''}`}
                    placeholder={isRateLimited ? "Daily limit reached" : "example.com"}
                    type="text"
                    value={url}
                    onChange={(e) => {
